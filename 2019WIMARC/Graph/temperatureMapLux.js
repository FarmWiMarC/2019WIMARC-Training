...
var TemperatureMapLux = function (ctx) {
    'use strict';
    this.ctx = ctx;
    this.points = [];
    this.polygon = [];
...
TemperatureMapLux.crossProduct = function (o, a, b) {
    'use strict';
    return (a.x - o.x) * (b.y - o.y) - (a.y - o.y) * (b.x - o.x);
};

TemperatureMapLux.pointInPolygon = function (point, vs) {
    'use strict';
    var x = point.x,
        y = point.y,
        inside = false,
...
TemperatureMapLux.squareDistance = function (p0, p1) {
    'use strict';
    var x = p0.x - p1.x,
        y = p0.y - p1.y;
...
TemperatureMapLux.hslToRgb = function (h, s, l) {
    'use strict';
...
TemperatureMapLux.prototype.getColor = function (levels, value) {
    'use strict';
    var val = value,
        tmp = 0,
        lim = 0.55,
  ...
  dif = max - min,
	...
	//console.log(tmp);
return TemperatureMapLux.hslToRgb(tmp, 1, 0.5);
   // return TemperatureMapLux.hslToRgb(tmp, 1, 0.5);
};

TemperatureMapLux.prototype.getPointValue = function (limit, point) {
    'use strict';
    var counter = 0,
        arr = [],
        tmp = 0.0,
    ...
    if (TemperatureMapLux.pointInPolygon(point, this.polygon)) {

        for (counter = 0; counter < this.points.length; counter = counter + 1) {
            dis = TemperatureMapLux.squareDistance(point, this.points[counter]);
            if (dis === 0) {
...
TemperatureMapLux.prototype.setConvexhullPolygon = function (points) {
    'use strict';
    var lower = [],
        upper = [],
        i = 0;
    ...
    for (i = 0; i < points.length; i = i + 1) {
        while (lower.length >= 2 && TemperatureMapLux.crossProduct(lower[lower.length - 2], lower[lower.length - 1], points[i]) <= 0) {
            lower.pop();
        }
        lower.push(points[i]);
    }
    ...
    for (i = points.length - 1; i >= 0; i = i - 1) {
        while (upper.length >= 2 && TemperatureMapLux.crossProduct(upper[upper.length - 2], upper[upper.length - 1], points[i]) <= 0) {
            upper.pop();
        }
        upper.push(points[i]);
    }
...
TemperatureMapLux.prototype.setPoints = function (arr, width, height) {
    'use strict';
    this.points = arr;
    this.width = width;
    this.height = height;
...
TemperatureMapLux.prototype.setRandomPoints = function (points, width, height) {
    'use strict';
    var counter = 0,
        x = 0,
        y = 0,
...
TemperatureMapLux.prototype.drawLow = function (limit, res, clean, callback) {
    'use strict';
    var self = this,
        ctx = this.ctx,
        dbl = 2 * res;
        ...
        for (x = xBeg; x < xEnd; x = x + res) {
            for (y = yBeg; y < yEnd; y =  y + res) {
                ...
                if (val !== -255) {
                    ...
                    gradient.addColorStop(0, str + '0.1)');
...
TemperatureMapLux.prototype.drawFull = function (levels, callback) {
    'use strict';
    var self = this,
        ctx = this.ctx,
        ...
        recursive = function () {
            window.requestAnimationFrame(function (timestamp) {
              ...
              //  bucleSteps = (bucleSteps << 3) / tDif;
                bucleSteps = (bucleSteps << 10) / tDif;
			 //  console.log(bucleSteps);
...
TemperatureMapLux.prototype.drawPoints = function (callback) {
    'use strict';
    var self = this,
        PI2 = 2 * Math.PI,
        ctx = this.ctx;
